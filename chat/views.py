...
def chat_list(request, user_name):
    ...
    message_list = MessageNotification.objects.filter(
        for_user=user).order_by('-notification_time')
...
def chat_room(request, chat_for_user):
    ...
    other_user_msgs = Messages.objects.filter(
        to_user=chat_user, from_user=request.user)
    print(f'other user {chat_user} message', other_user_msgs)

    notification = MessageNotification.objects.get(
        for_user=request.user, notification=chat_user)
